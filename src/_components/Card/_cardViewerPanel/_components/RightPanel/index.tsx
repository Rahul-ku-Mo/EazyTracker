...
const RightPanel = () => {
  ...
  return (
    <div className="h-full flex flex-col bg-white dark:bg-zinc-900 rounded-lg">
      <div className="py-4 pr-4 flex flex-col gap-4 overflow-y-auto flex-1 ">
    {/* Task Details Header */}
    <h2 className="text-sm font-semibold border border-border rounded-md py-1.5 px-2 dark:bg-zinc-800">
    ...
    {/* Priority Section */}
    <div className="space-y-3 pb-4 border border-border rounded-md py-1.5 px-2 dark:bg-zinc-800">
          ...
          <div className="space-y-2">
            {assignedMembers.map((assignedMember) => (
              <div key={assignedMember.id} className="group relative">
                <div className="flex items-center gap-2 p-2 bg-muted/30 hover:bg-muted/50 border border-border rounded-md transition-all duration-200 dark:border-zinc-600">
    ...
    <div className="space-y-3 pb-4 border border-border rounded-md py-1.5 px-2 dark:bg-zinc-800">
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs font-semibold text-primary">Additional Info</span>
      </div>

      <div className="space-y-3 text-xs">
        <div className="flex justify-between items-center">
          <span className="text-muted-foreground text-xs">Created</span>
          <span className="text-xs font-medium">{formatDate(createdAt) || "Mar 24, 2025"}</span>
        </div>
        ...
        <div className="flex justify-between items-center">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              ...
              >
                <Triangle strokeWidth={3} className="w-3 h-3 fill-current" />
                {storyPoints === 0 ? "No estimate" : `${storyPoints} Point${storyPoints !== 1 ? 's' : ''}`}
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="w-40">
              ...
              >
                <div className="flex items-center gap-2">
                  <Triangle strokeWidth={3} className="w-3 h-3 fill-current text-muted-foreground" />
                  <span>No estimate</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-muted-foreground">0</span>
                  {storyPoints === 0 && <Check strokeWidth={3} className="w-3 h-3 text-green-500" />}
                </div>
              </DropdownMenuItem>
              {[1, 2, 4, 8, 16].map((points) => (
                ...
                >
                  ...
                  {storyPoints === points && <Check strokeWidth={3} className="w-3 h-3 text-green-500" />}
                </DropdownMenuItem>
              ))}
            </DropdownMenuContent>
          </DropdownMenu>
    ...
    <div className="space-y-3 pb-4 border border-border rounded-md py-1.5 px-2 dark:bg-zinc-800">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-primary">Time Tracking</span>
        </div>
        <Button
          variant="ghost"
          size="sm"
